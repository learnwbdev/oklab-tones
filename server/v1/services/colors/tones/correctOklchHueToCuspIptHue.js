...
export function correctOklchHueToCuspIptHue(colorOklch, colorSpaceGamut = colorSpaces.sRgb, {mpPrcH = 2} = {}) {
    ...
    // find approx. L_cusp, C_cusp point for a given gamut for the Hue of colorOklch color
    // target Hue would be Hue in IPT which correcpond to the Hue for colorOklch at cusp point for the gamut, for L_cusp, C_cusp
    const aNrm = Math.cos(toRadians(origOklchHue)); // a / C (Chroma)
    const bNrm = Math.sin(toRadians(origOklchHue)); // b / C (Chroma)
    const oklabNormAB = [aNrm, bNrm];
    const [L_cusp, C_cusp] = findOklabLCCuspPoint(...oklabNormAB, colorSpaceGamut);
    const colorOklchTgHue = [L_cusp, C_cusp, origOklchHue];
    const tgIptHue = convertColor(colorOklchTgHue, colorSpaces.Oklch, colorSpaces.IPTch)[2];
